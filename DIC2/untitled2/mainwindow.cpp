    ...
    ui(new Ui::MainWindow)
{
    ...
    lengthLineEdt->setText("500");widthLineEdt->setText("250");speckDiamLineEdt->setText("10");
    denseCoeff=1.0;
    speckCount=1000;
    //Creating the menus
    setMenusAndToolBars();
...
{
    speckDiamStr = speckDiamLineEdt->text();
...
void MainWindow::on_denseSpinBox_valueChanged(/*const QString &arg1*/)
{
    if((ui->denseSpinBox->value()-0)>0.000001){
        ...
        speckCountLineEdt->setText(speckCountStr);
    }
}

void MainWindow::on_sCountEdit_editingFinished()
{
    speckCountStr=speckCountLineEdt->text();
...
{
    qreal widgetWidth=canvas->width();
    qreal widgetHeight=canvas->height();
    if(pWidth > pLength){
        QMessageBox::information(this, "Width of the plate can't be greater than its length.", "The values will be swapped");
        float *temp;temp=new float;
        *temp=pWidth;pWidth=pLength;pLength=*temp;delete temp;
...
void MainWindow::setTheMainGrid()
{
    mainBox = new QVBoxLayout;
    this->setLayout(mainBox);
    mainBox->addWidget(canvas);
    bottomBox=new QHBoxLayout;
    leftGrid=new QGridLayout;middleGrid=new QGridLayout;rightGrid=new QGridLayout;
    leftGrid->addWidget(lengthLbl, 0, 0);leftGrid->addWidget(lengthLineEdt, 0, 1);
    leftGrid->addWidget(widthLbl, 1, 0);leftGrid->addWidget(widthLineEdt, 1, 1);
    leftGrid->addWidget(speckDiamLbl, 2, 0);leftGrid->addWidget(speckDiamLineEdt, 2,1);
    leftGrid->addWidget(speckCountLbl, 3, 0);leftGrid->addWidget(speckCountLineEdt, 3, 1);
}
